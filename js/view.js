	...
	this.displayQuestion = function(){
		...
		$.each(questionList.questions,function(index,question){
			var li = $('<li>'+question+'</li>'+
				'<input type="radio" id="'+index+'no" name="'+index+'" value="no" checked>No</input>'+
				'<input type="radio" id="'+index+'yes" name="'+index+'" value="yes"/>Yes</input>');
			viewObj.questionsContainer.append(li);
			
			$("input[name="+index+"]").click(function(event){
				var userAnswer = $('input[id="'+event.target.id+'"]:checked').val();
				if(userAnswer === 'yes'){					
